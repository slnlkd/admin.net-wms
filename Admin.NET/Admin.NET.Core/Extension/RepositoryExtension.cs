...
public static class RepositoryExtension
{
    ...
    public static ISugarQueryable<T> OrderBuilderToJoin<T>(this ISugarQueryable<T> queryable, BasePageInput pageInput, string prefix = "", string defaultSortField = "Id", bool descSort = true)
    {
        ...
        TypeAdapterConfig typeAdapterConfig = new();
        typeAdapterConfig.ForType<T, BasePageInput>().IgnoreNullValues(true);
        Mapper mapper = new(typeAdapterConfig);
        var nowPagerInput = mapper.Map<BasePageInput>(pageInput);

        if (!string.IsNullOrEmpty(nowPagerInput.Field))
        {
            nowPagerInput.Field = Regex.Replace(nowPagerInput.Field, @"[\s;()\-'@=/%]", "");
            var orderByDbName = queryable.Context.EntityMaintenance.GetDbColumnName<T>(nowPagerInput.Field);

            // 智能判断是否需要添加前缀
            string finalFieldName;
            if (orderByDbName.Contains("."))
            {
                // 如果字段名已包含表别名，则不添加前缀
                finalFieldName = iSqlBuilder.GetTranslationColumnName(orderByDbName);
            }
            else
            {
                // 如果字段名不包含表别名，则添加前缀
            ...
            orderStr = $"{finalFieldName} {(string.IsNullOrEmpty(nowPagerInput.Order) || nowPagerInput.Order.Equals(nowPagerInput.DescStr, StringComparison.OrdinalIgnoreCase) ? "Desc" : "Asc")}";
        }
