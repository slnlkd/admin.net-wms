...
public static class SqlSugarSetup
{
    ...
    public static void SetDbAop(SqlSugarScopeProvider db, bool enableConsoleSql, bool superAdminIgnoreIDeletedFilter)
    {
        ...
        if (enableConsoleSql)
        {
            db.Aop.OnLogExecuting = (sql, pars) =>
            {
                ...
                //优化sql语句显示
                var newSql = MergeStrSql(sql, pars);
    ...
    /// <returns></returns>
    private static string MergeStrSql(string strSql, SugarParameter[] pars)
    {
        var param = pars.Select(x => new { key = x.ParameterName, value = x.Value ,type = x.DbType});
        foreach (var item in param)
            try
            {
                if (item.value is not null && item.value != null)
                {
                    switch (item.type)
                    {
                        case System.Data.DbType.Binary:
                            strSql = strSql.Replace(item.key, $"{item.value}");
                            break;
                        ...
                        case System.Data.DbType.Boolean:
                            var valueBool = (Boolean)item.value ? "1" : "0";
                            strSql = strSql.Replace(item.key, $"{valueBool}");
                            break;
                ...
                {
                    strSql = strSql.Replace(item.key, $"null");
                }
            }
            catch (Exception ex)
            {
