...
public static class SqlSugarFilter
{
    ...
    public static void SetCustomEntityFilter(SqlSugarScopeProvider db)
    {
        ...
        SetCustomEntityFilter(db, userId);
    }

    /// <summary>
    /// 配置自定义过滤器
    /// </summary>
    public static void SetCustomEntityFilter(SqlSugarScopeProvider db, string userId)
    {
        if (string.IsNullOrEmpty(userId)) return;
        ...
        if (tableFilterItemList == null)
        {
            ...
            foreach (var entityFilter in entityFilterTypes)
            {
                ...
                // 修复：传递正确的参数
                var entityFilters = ((IList)entityFilterMethod?.Invoke(instance, new object[] { userId }))?.Cast<object>();
...
public interface IEntityFilter
{
    ...
    /// 实体过滤器
    /// </summary>
    /// <param name="userId">需要修改权限的用户ID</param>
    /// <returns></returns>
    IEnumerable<TableFilterItem<object>> AddEntityFilter(string userId);
}
