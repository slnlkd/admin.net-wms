...
public class SysRoleService : IDynamicApiController, ITransient
{
    ...
    /// <summary>
    /// 根据当前登录的用户查看快捷菜单(主页查询快捷菜单数据接口）
    /// </summary>
    /// <returns></returns>
    public async Task<List<SysMenu>> GetQuickActionList()
    {
        // 获取当前登录人的角色ID
        var sysUserRole = await _sysUserRoleRep.AsQueryable().Where(u => u.UserId == Convert.ToInt64(App.User.FindFirst(ClaimConst.UserId).Value)).SingleAsync();
        // 根据角色ID 渲染 快捷菜单
        var list = await _sysMenuQuickActionsRep.AsQueryable()
            .InnerJoin<SysRoleMenu>((u, rm) => u.MenuId == rm.MenuId)
            .InnerJoin<SysMenu>((u, rm, SysMenu) => u.MenuId == SysMenu.Id)
            .Where((u, rm, SysMenu) => u.RoleId == sysUserRole.RoleId && SysMenu.Pid != 0)
            .Select((u, rm, SysMenu) => SysMenu).ToListAsync();
        foreach (var item in list)
        {
            item.Icon = item.Icon.Replace("ele-","");
        }
        return list;
    }
    ...
    /// <param name="input"></param>
    /// <returns></returns>
    private async Task UpdateQuickActionMenu(AddRoleInput input)
    {
        // 更新快捷菜单
        await _sysRoleMenuService.GranQuickActionMenu(new RoleMenuInput()
        {
            Id = input.Id,
            MenuIdList = input.MenuIdList.ToList()
        });
    ...
    /// <param name="input"></param>
    /// <returns></returns>
    [ApiDescriptionSettings(Name = "UpdateQucikAction"), HttpPost]
    [DisplayName("更新角色快捷菜单")]
    public async Task UpdateQucikAction(UpdateRoleInput input)
    {
        ...
        await UpdateQuickActionMenu(input);
    }
    ...
    /// <returns></returns>
    [DisplayName("根据角色Id获取菜单Id集合")]
    public async Task<List<long>> GetQuickActionMenuList([FromQuery] RoleInput input)
    {
        var menuIds = await _sysRoleMenuService.GetRoleQuickActionMenuList(new List<long> { input.Id });
        return await _sysMenuService.ExcludeParentMenuOfFullySelectedQuickAction(menuIds);
    }
