...
//制作选择物料信息表格
export const useTableExportNotifyAddDetail = defineStore('useTableExportNotifyAddDetail', {
	state() {
		return {
			//被勾选框选中的数据
			...
			parentProps: {
				customerId: null,
				exportBillTypeId: null,
				exportChildBillTypeId: null,
				wareHouseId: null,
				wayOutCode: null,
			},
		...
		async SelectMaterialFun() {
			try {
				let newParam = {
					...
					customerId: this.parentProps.customerId,
					exportBillTypeId: this.parentProps.exportBillTypeId,
					exportChildBillTypeId: this.parentProps.exportChildBillTypeId,
					wareHouseId: this.parentProps.wareHouseId,
					wayOutCode: this.parentProps.wayOutCode,
				};

				let notifyResult = await axios.post(window.__env__.VITE_API_URL + '/api/wmsExportNotify/selectMaterial', newParam);
