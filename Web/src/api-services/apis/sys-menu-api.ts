...
export const SysMenuApiAxiosParamCreator = function (configuration?: Configuration) {
    return {
         ...
         * @throws {RequiredError}
         */
        apiSysRoleMenuListGet: async (roleId?: number, options: AxiosRequestConfig = {}): Promise<RequestArgs> => {
            const localVarPath = `/api/sysMenu/GetRoleMenuList`;
            // use dummy base URL string because the URL constructor only accepts absolute URLs.
            const localVarUrlObj = new URL(localVarPath, 'https://example.com');
            let baseOptions;
            if (configuration) {
...
export const SysMenuApiFp = function(configuration?: Configuration) {
    return {
         ...
         * @throws {RequiredError}
         */
        async apiSysRoleMenuListGet(roleId?: number, options?: AxiosRequestConfig): Promise<(axios?: AxiosInstance, basePath?: string) => Promise<AxiosResponse<AdminResultListSysMenu>>> {
            const localVarAxiosArgs = await SysMenuApiAxiosParamCreator(configuration).apiSysRoleMenuListGet(roleId, options);
            return (axios: AxiosInstance = globalAxios, basePath: string = BASE_PATH) => {
                const axiosRequestArgs :AxiosRequestConfig = {...localVarAxiosArgs.options, url: basePath + localVarAxiosArgs.url};
                return axios.request(axiosRequestArgs);
            };
...
export class SysMenuApi extends BaseAPI {
     ...
     * @memberof SysMenuApi
     */
    public async apiSysRoleMenuListGet(roleId?: number, options?: AxiosRequestConfig) : Promise<AxiosResponse<AdminResultListSysMenu>> {
        return SysMenuApiFp(this.configuration).apiSysRoleMenuListGet(roleId, options).then((request) => request(this.axios, this.basePath));
    }
    /**
     * 
     * @summary 获取登录菜单树 🔖
