...
const setDictData = () => {
  // 创建深拷贝，避免修改原始字典数据
  const rawData = getDataList()?.filter(props.onItemFilter) ?? [];
  state.dictData = rawData.map(item => ({ ...item }));
  processValueTypes(props.modelValue);
};

// 处理值类型匹配，确保字典数据的 value 与传入值类型一致
const processValueTypes = (value: any) => {
  ...
  if (typeof firstValue === 'number') {
    ...
    state.dictData.forEach((item) => {
      item[props.propValue] = String(item[props.propValue]);
    });
  }
};
