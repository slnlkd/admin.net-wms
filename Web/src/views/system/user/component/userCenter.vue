				...
				<el-card shadow="hover">
					<el-tabs>
						...
						<el-tab-pane label="修改密码">
							<ChangePassword
								style="width: 50%;" 
								:user-name="userInfos.account"
								:is-admin="userInfos.accountType == 999"
								:require-old-password="true"
								:show-auth="true"
								auth-code="sysUser:changePwd"
...
import { accessTokenKey, getAPI } from '/@/utils/axios-utils';
...
onMounted(async () => {
	...
	state.loading = false;
	// 获取密码策略
	const policyRes = await wmsPasswordPolicyApi.getEnableDetail().then(res => res.data.result);
	pwdPolicy.value = policyRes;
});
