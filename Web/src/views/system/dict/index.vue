				...
				<el-card class="full-table" shadow="hover" :body-style="{ height: 'calc(100% - 51px)' }">
					...
					<el-form :model="state.queryDictTypeParams" ref="queryForm" :inline="true" @submit.native.prevent
						...
						<el-form-item label="编码">
							<el-input v-model="state.queryDictTypeParams.code" @keyup.enter.native="handleDictTypeQuery"
								placeholder="字典编码" clearable />
						</el-form-item>
						<el-form-item>
							<el-button-group>
					...
					<el-table :data="state.dictTypeData" style="width: 100%" v-loading="state.typeLoading"
						@row-click="handleDictType" highlight-current-row @sort-change="sortChangeDityType" border>
						...
						<el-table-column prop="code" label="字典编码" min-width="140" header-align="center"
							sortable='custom' show-overflow-tooltip />
						<el-table-column prop="sysFlag" label="系统内置" min-width="90" align="center" sortable='custom'
							show-overflow-tooltip v-if="userInfo.accountType === AccountTypeEnum.NUMBER_999">
							<template #default="scope">
								<g-sys-dict v-model="scope.row.sysFlag" code="YesNoEnum" />
							</template>
						</el-table-column>
						<el-table-column prop="isTenant" label="租户字典" min-width="90" align="center" sortable='custom'
							show-overflow-tooltip v-if="userInfo.accountType === AccountTypeEnum.NUMBER_999">
							<template #default="scope">
						...
						<el-table-column prop="orderNo" label="排序" width="80" align="center" sortable='custom'
							show-overflow-tooltip />
						...
						<el-table-column label="操作" width="120" fixed="right" align="center"
							v-if="auths(['sysDictType:update', 'sysDictType:delete'])">
							<template #default="scope">
								<el-tooltip content="编辑">
									<el-button icon="ele-Edit" size="small" text type="primary"
										:disabled="!hasPermission(scope.row)" @click="openEditDictType(scope.row)"
										v-auth="'sysDictType:update'"> </el-button>
								</el-tooltip>
								<el-tooltip content="删除">
									<el-button icon="ele-Delete" size="small" text type="danger"
										:disabled="scope.row?.sysFlag === 1 || !hasPermission(scope.row)"
				...
				<el-card class="full-table" shadow="hover" :body-style="{ height: 'calc(100% - 51px)' }">
					...
					<el-form :model="state.queryDictDataParams" ref="queryForm" :inline="true" @submit.native.prevent
						...
						<el-form-item label="显示文本">
							<el-input v-model="state.queryDictDataParams.label" placeholder="显示文本"
						...
						<el-form-item>
							<el-button type="primary" icon="ele-Plus" @click="openAddDictData"
								:disabled="!hasPermission(state.selectDict)" v-auth="'sysDictData:add'"> 新增 </el-button>
						</el-form-item>
					</el-form>

					<el-table :data="state.dictDataData" style="width: 100%" v-loading="state.loading" border>
						<el-table-column type="index" label="序号" width="55" align="center" />
						<el-table-column prop="value" label="显示文本" header-align="center" min-width="120"
							show-overflow-tooltip>
							<template #default="scope">
								<el-tag :type="scope.row.tagType" :style="scope.row.styleSetting"
									:class="scope.row.classSetting">{{ scope.row.label }}</el-tag>
							</template>
						</el-table-column>
						...
						<el-table-column label="操作" width="120" fixed="right" align="center" show-overflow-tooltip
							v-if="auths(['sysDictData:add', 'sysDictData:update', 'sysDictData:delete'])">
							<template #default="scope">
								<el-tooltip content="编辑">
									<el-button icon="ele-Edit" size="small" text type="primary"
										@click="openEditDictData(scope.row)" v-auth="'sysDictData:update'"> </el-button>
								</el-tooltip>
								<el-tooltip content="删除">
									<el-button icon="ele-Delete" size="small" text type="danger"
										:disabled="!hasPermission(state.selectDict)" @click="delDictData(scope.row)"
										v-auth="'sysDictData:delete'"> </el-button>
								</el-tooltip>
								<el-tooltip content="复制">
									<el-button icon="ele-CopyDocument" size="small" text type="primary"
										:disabled="!hasPermission(state.selectDict)"
										@click="openCopyDictData(scope.row)" v-auth="'sysDictData:add'"> </el-button>
								</el-tooltip>
							</template>
						</el-table-column>
					</el-table>
					<el-pagination v-model:currentPage="state.tableDictDataParams.page"
						...
						:page-sizes="[10, 20, 50, 100]" size="small" background @size-change="handleDictDataSizeChange"
