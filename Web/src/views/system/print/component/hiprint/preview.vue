<template>
	<el-dialog v-model="state.dialogVisible" draggable :close-on-click-modal="false"
		:width="Number(state.width) + Number(8) + 'mm'">
		<template #header>
			<div style="color: #fff">
		...
		<template #footer>
			<el-button :loading="state.waitShowPrinter" type="primary" icon="ele-Printer"
				@click.stop="print">直接打印</el-button>
...
const showDialog = (hiprintTemplate: any, printData: {}, width = 210, printType = 1, printParam: { printer: ''; title: ''; color: false; copies: 1 }) => {
	...
	if (state.hiprintTemplate.lastJson.type == "import") {
		importprint();
	}

};
const importprint = async () => {
	//入库明细打印
	var params = { importDetailId: state.printData[0].importDetailId, isprint: '1', number: state.printData[0].number };
	console.log(params);
	var printDataRes = await wmsImportLabelPrintApi.print(params);
	state.printData = printDataRes.data.result;
