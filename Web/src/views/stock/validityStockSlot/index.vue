...
onMounted(async () => {
  ...
  const result = await wmsStockWarningApi.GetWarningConfig('sys_validity_waring_day').then(res => res.data.result);
...
const handleQuery = async (params: any = {}) => {
  state.tableQueryParams.type="Validity";
  
  if (state.warningDay === undefined || state.warningDay === null|| state.warningDay === 0) {
    const result = await wmsStockWarningApi.GetWarningConfig('sys_validity_waring_day').then(res => res.data.result);
    state.warningDay=result;
  }
  ...
  const result = await wmsStockWarningApi.ValidityExpireWarning(Object.assign(state.tableQueryParams, state.tableParams)).then(res => res.data.result);
...
const warningConfig= async()=>{
  await wmsStockWarningApi.WarningConfig({code:'sys_validity_waring_day',value:state.warningDay}).then(res => res.data.result);
...
const exportWmsReportCommand = async (command: string) => {
  try {
     state.tableQueryParams.WarningType=2;
...
<template>
  <div v-loading="state.exportLoading">
          ...
          <el-col :xs="24" :sm="12" :md="12" :lg="8" :xl="4" class="mb10">
            <el-form-item >
              <el-button-group style="display: flex; align-items: center;">
                <el-button type="primary"  icon="ele-Search" @click="handleQuery" v-auth="'wmsValidityStockSlot:page'" v-reclick="1000"> 查询 </el-button>
                <el-button icon="ele-Refresh" @click="() => state.tableQueryParams = {}"> 重置 </el-button>
                
                <!-- <el-button type="primary" icon="ele-Plus" @click="editDialogRef = true" style="margin-left: 10px">效期预警设置</el-button> -->
                ...
                <el-dropdown :show-timeout="70" :hide-timeout="50" @command="exportWmsReportCommand">
                  <el-button type="primary" style="margin-left:5px;" icon="ele-FolderOpened" v-reclick="20000" v-auth="'wmsValidityStockSlot:export'"> 导出 </el-button>
                  <template #dropdown>
                    <el-dropdown-menu>
    ...
    <el-dialog v-model="editDialogRef" draggable title="近效期预警天数设置" width="500">
    <!-- <el-form :model="form"> -->
    <el-form>
