...
const handleQuery = async (params: any = {}) => {
  ...
  const result = await wmsStockWarningApi.StockLowWarning(Object.assign(state.tableQueryParams, state.tableParams)).then(res => res.data.result);
...
const exportWmsReportCommand = async (command: string) => {
  try {
    ...
    state.tableQueryParams.WarningType=4;
    if (command === 'select') {
  ...
  <div v-loading="state.exportLoading">
          ...
          <el-col :xs="24" :sm="12" :md="12" :lg="8" :xl="4" class="mb10">
            <el-form-item >
              <el-button-group style="display: flex; align-items: center;">
                <el-button type="primary"  icon="ele-Search" @click="handleQuery" v-auth="'wmsLowStockSlot:page'" v-reclick="1000"> 查询 </el-button>
                <el-button icon="ele-Refresh" @click="() => state.tableQueryParams = {}"> 重置 </el-button>
                
                <el-dropdown :show-timeout="70" :hide-timeout="50" @command="exportWmsReportCommand">
                  <el-button type="primary" style="margin-left:5px;" icon="ele-FolderOpened" v-reclick="20000" v-auth="'wmsLowStockSlot:export'"> 导出 </el-button>
                  <template #dropdown>
                    <el-dropdown-menu>
    ...
    <el-card class="full-table" shadow="hover" style="margin-top: 5px">
      <el-table :data="state.tableData" @selection-change="(val: any[]) => { state.selectData = val; }" style="width: 100%" v-loading="state.tableLoading" tooltip-effect="light" row-key="id" @sort-change="sortChange" border>
        ...
        <el-table-column prop='materialStockLow' label='最低储量' show-overflow-tooltip />
        <el-table-column prop='materialLowCount' label='低出数量' show-overflow-tooltip />
