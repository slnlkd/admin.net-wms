...
import { useWmsExportOrderApi } from '/@/api/count/wmsExportOrder';
...
const handleQuery = async (params: any = {}) => {
  ...
  const result = await wmsExportOrderApi.GetSearchExOrder(Object.assign(state.tableQueryParams, state.tableParams)).then(res => res.data.result);
...
const exportWmsReportCommand = async (command: string) => {
  try {
    ...
    await wmsExportOrderApi.ExExport(params).then(res => downloadStreamFile(res));
  } finally {
...
const handleDetailRowDblClick = (row, column, event) => {
  ...
  handleBoxQuery({'MaterialCode':row.exportMaterialCode,'LotNo':row.exportLotNo,'ExBillCode':row.exportBillCode});
}
...
const handleDetailQuery = async (params: any = {}) => {
  ...
  state.tableDetailQueryParams.exportSlotCode=state.tableQueryParams.exportSlotCode;
  state.tableDetailQueryParams.exportStockCode=state.tableQueryParams.exportStockCode;
  state.tableDetailQueryParams.exBillCode=state.tableQueryParams.exBillCode;
  ...
  const result = await wmsExportOrderApi.GetSearchExOrderDetail(Object.assign(state.tableDetailQueryParams, state.tableDetailParams)).then(res => res.data.result);
...
const handleBoxQuery = async (params: any = {}) => {
  ...
  const result = await wmsExportOrderApi.GetSearchExOrderBoxInfo(Object.assign(state.tableBoxQueryParams, state.tableBoxParams)).then(res => res.data.result);
        ...
        <el-row>
          ...
          <el-col :xs="24" :sm="12" :md="12" :lg="8" :xl="4" class="mb10" v-if="state.showAdvanceQueryUI">
            <el-form-item label="出库单">
                <el-input v-model="state.tableQueryParams.exBillCode" clearable placeholder="请输入出库单"/>
            </el-form-item>
          </el-col>
          ...
          <el-col :xs="24" :sm="12" :md="8" :lg="6" :xl="4" class="mb10" v-if="state.showAdvanceQueryUI">
            ...
            <el-col :xs="24" :sm="12" :md="8" :lg="6" :xl="4" class="mb10" v-if="state.showAdvanceQueryUI">
            <el-form-item label="箱码编号">
              <el-input v-model="state.tableQueryParams.boxCode" clearable placeholder="请输入箱码编号" @keyup.enter="handleQuery" />
            </el-form-item>
          </el-col>
          <el-col :xs="24" :sm="12" :md="12" :lg="8" :xl="4" class="mb10">
            <el-form-item >
              <el-button-group style="display: flex; align-items: center;">
                <el-button type="primary"  icon="ele-Search" @click="handleQueryAll" v-auth="'wmsExportOrderCount:page'" v-reclick="1000"> 查询 </el-button>
                ...
                <el-dropdown :show-timeout="70" :hide-timeout="50" @command="exportWmsReportCommand">
                  <el-button type="primary" style="margin-left:5px;" icon="ele-FolderOpened" v-reclick="20000" v-auth="'wmsExportOrderCount:export'"> 导出 </el-button>
                  <template #dropdown>
                    <el-dropdown-menu>
          ...
          <el-tabs v-model="activeName" @tab-click="handleClick">
            <el-tab-pane label="总单" name="first">
              <div v-if="activeName=='first'" >
                <el-table  ref="importTableRef" :data="state.tableData" @selection-change="(val: any[]) => { state.selectData = val; }"  
                  highlight-current-row @row-dblclick="handleRowDblClick" style="width: 100%" v-loading="state.tableLoading" tooltip-effect="light" row-key="materialCode" @sort-change="sortChange" border>
                    ...
                    <el-table-column prop='qty' label='出库数量' show-overflow-tooltip />
            ...
            <el-tab-pane label="明细" name="second">
                <el-table :data="state.tableDetailData" @selection-change="(val: any[]) => { state.selectData = val; }" @row-dblclick="handleDetailRowDblClick"  highlight-current-row style="width: 100%" v-loading="state.tableLoading" tooltip-effect="light" row-key="id" @sort-change="sortChange" border>
                    <el-table-column type="selection" width="40" align="center"/>
                    <el-table-column type="index" label="序号" width="55" align="center"/>
                    <el-table-column prop='exportOrderNo' label='出库流水号' show-overflow-tooltip />
                    <el-table-column prop='exportBillCode' label='出库单号' show-overflow-tooltip />
                    <el-table-column prop='exportStockCode' label='载具条码' show-overflow-tooltip />
                    <el-table-column prop='exportSlotCode' label='储位地址' show-overflow-tooltip />
                    <el-table-column prop='exportLotNo' label='批次' show-overflow-tooltip />
                    <el-table-column prop='pickedNum' label='出库数量' show-overflow-tooltip />
                    <el-table-column prop='exportExecuteFlag' label='执行状态' show-overflow-tooltip />
            ...
            <el-tab-pane label="箱码" name="third">
                <el-table :data="state.tableBoxData" @selection-change="(val: any[]) => { state.selectData = val; }" style="width: 100%" v-loading="state.tableLoading" tooltip-effect="light" row-key="id" @sort-change="sortChange" border>
                    ...
                    <el-table-column prop='qty' label='箱内数量' show-overflow-tooltip />
                    <el-table-column prop='pickEdNum' label='实际数量' show-overflow-tooltip />
                    <el-table-column prop='productionDate' label='生产日期' show-overflow-tooltip />
                    <el-table-column prop='exportLoseDate' label='有效期' show-overflow-tooltip />
                </el-table>
                <el-pagination 
