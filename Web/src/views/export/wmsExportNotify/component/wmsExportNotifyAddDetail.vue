                ...
                <el-card class="full-table" shadow="hover" :body-style="{ height: 'calc(100% - 51px)' }">
                    <el-form :inline="true" @submit.native.prevent class="handle-form">
                        <el-form-item label="关键字：">
                            <el-input v-model="useTableExportNotifyAddDetailStore.whereStrParam" placeholder="关键字"></el-input>
                        </el-form-item>
                        <el-form-item>
                            <el-button type="primary" icon="ele-Search"
                                @click="useTableExportNotifyAddDetailStore.SelectMaterialFun()" v-auth="'sysDifflog:page'"> 查询
                            </el-button>
                        </el-form-item>
                        <el-form-item>
                    ...
                    <el-table style="width: 100%"  highlight-current-row @sort-change="" border
                        :data="useTableExportNotifyAddDetailStore.displayNotifyListComputed" @row-click="" @selection-change="useTableExportNotifyAddDetailStore.SelectRowReturnFun">
                        ...
                        <el-table-column prop="importProductionDate" label="生产日期" min-width="150" header-align="center"  align="center"/>
                        <el-table-column prop="importLostDate" label="失效日期" min-width="150" header-align="center"  align="center"/>
                    </el-table>
                    <el-pagination v-model:currentPage="useTableExportNotifyAddDetailStore.page"
                        v-model:page-size="useTableExportNotifyAddDetailStore.pageSize" :total="useTableExportNotifyAddDetailStore.total"
...
<script lang="ts" name="wmsExportNotifyAddDetail" setup>
...
onMounted(async () => {
    try {
        useTableExportNotifyAddDetailStore.SelectMaterialFun();
...
const handleDictTypeSizeChange = (val: number) => {
  useTableExportNotifyAddDetailStore.pageSize = val;
  useTableExportNotifyAddDetailStore.SelectMaterialFun();
};

// 改变页码序号
const handleDictTypeCurrentChange = (val: number) => {
  useTableExportNotifyAddDetailStore.page = val;
  useTableExportNotifyAddDetailStore.SelectMaterialFun();
};
