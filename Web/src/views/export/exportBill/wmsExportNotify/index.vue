      ...
      <el-col :span="24" :xs="24" style="display: flex; height: 100%; flex: 1">
        <el-card class="full-table" shadow="hover" :body-style="{ height: 'calc(100% - 51px)' }">
          ...
          <el-form :inline="true" @submit.native.prevent class="handle-form">
            ...
            <el-form-item>
              <el-button type="primary" icon="ele-Plus" @click="addNotifyFun" v-auth="'wmsExportNotify:add'"> 新增 </el-button>
            </el-form-item>
          </el-form>
          ...
          <el-table style="width: 100%" @row-click="useTableExportNotifyStore.ShowNotifyAndDetiailByNotifyIdFun"
            ...
            <el-table-column prop="warehouseId" label="所属仓库" min-width="200" align="center" header-align="center" v-if="false"/>
            ...
            <el-table-column label="操作" width="300" fixed="right" align="center" header-align="center">
              <template #default="scope">
                <el-button icon="ele-Check" size="small" text type="primary" v-auth="'wmsExportNotify:ZDFP'" @click="useTableExportNotifyStore.deleteExportTaskAuto(scope.row)"
                  > 自动分配</el-button>
                  <el-button icon="ele-Delete" style="margin-left:5px;"  size="small" text type="danger" v-auth="'wmsExportNotify:delete'"
                    @click="useTableExportNotifyStore.deleteRow(scope.row)">删除 </el-button>
              </template>
            </el-table-column>
          </el-table>
        ...
        <el-card class="full-table" shadow="hover" :body-style="{ height: 'calc(100% - 51px)' }">
          ...
          <el-table style="width: 100%" @row-click="" highlight-current-row @sort-change="" border
            ...
            <el-table-column prop="completeQuantity" label="完成数量" min-width="300" align="center"
              header-align="center" />
            <el-table-column prop="" label="操作" min-width="170" align="center" header-align="center" fixed="right">
              <template #default="scope">
              
                <el-button icon="ele-Check" size="small" text type="primary" v-auth="'wmsExportNotify:SDFP'" @click="selectStockSlot(scope.row)"
                  > 手动分配</el-button>
                  <el-button icon="ele-Delete" size="small" text type="danger" @click="useTableExportNotifyStore.deleteDetialRow(scope.row)"
                    v-auth="'wmsExportNotify:delete'" style="margin-left:5px;"> 删除</el-button>
              </template>
            </el-table-column>
          </el-table>
...
//打开弹窗“添加手动分配界面”
function selectStockSlot(row: any) {
  useTablewmsExportStockSlotStore.parentProps.notifyDetailId = row.notifyDetailId;
        
  useChildOpenAndCloseStore.open_wmsExportStockSlot_C_child_1();
}
//打开弹窗“添加出库单据页面”
