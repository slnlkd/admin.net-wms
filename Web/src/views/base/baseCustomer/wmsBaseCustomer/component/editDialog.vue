<script lang="ts" name="wmsBaseCustomer" setup>
...
const openDialog = async (row: any, title: string) => {
	state.title = title;
	row = row ?? {  };
	state.ruleForm = row.id ? await wmsBaseCustomerApi.detail(row.id).then(res => res.data.result) : JSON.parse(JSON.stringify(row));
...
const submit = async () => {
	ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
		if (isValid) {
			...
			await wmsBaseCustomerApi[state.ruleForm.id ? 'update' : 'add'](values);
			closeDialog();
		} else {
			ElMessage({
...
<template>
	<div class="wmsBaseCustomer-container">
