...
const approvalMaterial = (row: any, approvalStatus: any) => {
  ...
  if (approvalStatus === 2) {
    ...
    ElMessageBox.prompt('请输入驳回原因', '驳回申请', {
      ...
      try {
        await wmsBaseMaterialPresetApi.approvalMaterial(row.id, approvalStatus, value);
        // 发送消息通知给申请人
...
// 批量审核物料信息
const batchApprovalMaterial = (approvalStatus: any) => {
  ...
  if (approvalStatus === 2) {
    ...
    ElMessageBox.prompt('请输入驳回原因', '批量驳回申请', {
      ...
      try {
        await wmsBaseMaterialPresetApi.batchApprovalMaterial(ids, approvalStatus, value);
        // 批量发送消息通知给申请人
        for (const row of state.selectedRows) {
          await sendApprovalNotification(row, approvalStatus);
        }
        ElMessage.success(`成功${actionText} ${state.selectedRows.length} 条记录`);
        state.selectedRows = [];
        handleQuery(); // 重新查询
        emit('refresh'); // 通知父组件刷新待审核数量
      } catch (error: any) {
    ...
    ElMessageBox.confirm(`确定要${actionText}选中的 ${state.selectedRows.length} 条记录吗?`, "提示", {
      ...
      try {
        await wmsBaseMaterialPresetApi.batchApprovalMaterial(ids, approvalStatus);
...
const sendApprovalNotification = async (row: any, approvalStatus: any) => {
  try {
    ...
    const noticeData: AddNoticeInput = {
      ...
      content: `申请人【${applicantName}】申请的物料信息【${materialName}】（编码：${row.materialCode}）已${statusText}。`,
      type: 1, // 通知类型：通知
      status: 1, // 已发布状态
    ...
    // 注意：这里假设添加通知时如果status=1会自动发布
    // 如果不行，可能需要调用发布接口，但需要通知ID
...
const emit = defineEmits(['refresh', 'refreshMaterialList']);
            ...
            <el-col :xs="24" :sm="12" :md="12" :lg="8" :xl="6" class="mb10">
              <el-form-item>
                <el-button-group style="display: flex; align-items: center;">
                  <el-button type="primary" icon="ele-Search" @click="handleQuery" v-auth="'wmsBaseMaterialPreset:page'"
      ...
      <el-card class="full-table" shadow="hover" style="margin-top: 5px">
        <div class="batch-actions">
          ...
          <div class="batch-actions-right">
            <el-button-group>
              <el-button type="success" icon="ele-Check" @click="batchApprovalMaterial(1)"
                v-auth="'wmsBaseMaterialPreset:auth'" :disabled="state.selectedRows.length === 0">
                批量通过
              </el-button>
              <el-button type="danger" icon="ele-Close" @click="batchApprovalMaterial(2)"
                v-auth="'wmsBaseMaterialPreset:auth'" :disabled="state.selectedRows.length === 0">
                批量驳回
              </el-button>
        ...
        <el-table :data="state.tableData" style="width: 100%" v-loading="state.tableLoading" tooltip-effect="light"
          ...
          <el-table-column :fixed="true" align="center" width="155" prop='materialCode' label='物料编码'
            show-overflow-tooltip />
          <el-table-column :fixed="true" align="center" width="155" prop='materialType' label='物料类型'
            show-overflow-tooltip>
          ...
          <el-table-column align="center" prop='materialUnit' label='计量单位'
            :formatter="(row: any) => row.materialUnitFkDisplayName" show-overflow-tooltip />
          <el-table-column align="center" prop='materialAreaId' label='专属区域'
            :formatter="(row: any) => row.materialAreaFkDisplayName" show-overflow-tooltip />
          ...
          <el-table-column align="center" width="155" prop='warehouseId' label='所属仓库'
            :formatter="(row: any) => row.warehouseFkDisplayName" show-overflow-tooltip />
          <el-table-column label="操作" width="140" align="center" fixed="right" show-overflow-tooltip
            v-if="auth('wmsBaseMaterialPreset:auth')">
            <template #default="scope">
              <el-button icon="ele-Check" size="small" text type="success" @click="approvalMaterial(scope.row, 1)"
                v-auth="'wmsBaseMaterialPreset:auth'"> 通过 </el-button>
              <el-button icon="ele-Close" size="small" text type="danger" @click="approvalMaterial(scope.row, 2)"
                v-auth="'wmsBaseMaterialPreset:auth'"> 驳回 </el-button>
            </template>
          </el-table-column>
        </el-table>
