...
// 审核客户信息
const approvalCustomer = (row: any, approvalStatus: any) => {
  ...
  if (approvalStatus === 2) {
    ...
    ElMessageBox.prompt('请输入驳回原因', '驳回申请', {
      ...
      try {
        await wmsBaseCustomerPresetApi.approvalCustomer(row.id, approvalStatus, value);
        // 发送消息通知给申请人
    ...
    ElMessageBox.confirm(`确定要${actionText}吗?`, "提示", {
      ...
      try {
        await wmsBaseCustomerPresetApi.approvalCustomer(row.id, approvalStatus);
...
// 批量审核客户信息
const batchApprovalCustomer = (approvalStatus: any) => {
  ...
  if (approvalStatus === 2) {
    ...
    ElMessageBox.prompt('请输入驳回原因', '批量驳回申请', {
      ...
      try {
        await wmsBaseCustomerPresetApi.batchApprovalCustomer(ids, approvalStatus, value);
        // 批量发送消息通知给申请人
    ...
    ElMessageBox.confirm(`确定要${actionText}选中的 ${state.selectedRows.length} 条记录吗?`, "提示", {
      ...
      try {
        await wmsBaseCustomerPresetApi.batchApprovalCustomer(ids, approvalStatus);
...
const sendApprovalNotification = async (row: any, approvalStatus: any) => {
  try {
    const customerName = row.customerName || row.customerCode || '未知客户';
    ...
    const noticeData: AddNoticeInput = {
      title: `客户信息审核${statusText}`,
      content: `申请人【${applicantName}】申请的客户信息【${customerName}】（编码：${row.customerCode}）已${statusText}。`,
      type: 1, // 通知类型：通知
      status: 1, // 已发布状态
...
const emit = defineEmits(['refresh', 'refreshCustomerList']);
            ...
            <el-col :xs="24" :sm="12" :md="12" :lg="8" :xl="6" class="mb10">
              <el-form-item>
                <el-button-group style="display: flex; align-items: center;">
                  <el-button type="primary" icon="ele-Search" @click="handleQuery" v-auth="'wmsBaseCustomerPreset:page'"
      ...
      <el-card class="full-table" shadow="hover" style="margin-top: 5px">
        <div class="batch-actions">
          ...
          <div class="batch-actions-right">
            <el-button-group>
              <el-button type="success" icon="ele-Check" @click="batchApprovalCustomer(1)"
                v-auth="'wmsBaseCustomerPreset:auth'" :disabled="state.selectedRows.length === 0">
                批量通过
              </el-button>
              <el-button type="danger" icon="ele-Close" @click="batchApprovalCustomer(2)"
                v-auth="'wmsBaseCustomerPreset:auth'" :disabled="state.selectedRows.length === 0">
                批量驳回
              </el-button>
        ...
        <el-table :data="state.tableData" style="width: 100%" v-loading="state.tableLoading" tooltip-effect="light"
          ...
          <el-table-column :fixed="true" align="center" width="155" prop='customerCode' label='客户编码'
            show-overflow-tooltip />
          <el-table-column align="center" width="200" prop='customerName' label='客户名称' show-overflow-tooltip />
          <el-table-column align="center" width="200" prop='customerAddress' label='客户地址' show-overflow-tooltip />
          ...
          <el-table-column label="操作" width="140" align="center" fixed="right" show-overflow-tooltip
            v-if="auth('wmsBaseCustomerPreset:auth')">
            <template #default="scope">
              <el-button icon="ele-Check" size="small" text type="success" @click="approvalCustomer(scope.row, 1)"
                v-auth="'wmsBaseCustomerPreset:auth'"> 通过 </el-button>
              <el-button icon="ele-Close" size="small" text type="danger" @click="approvalCustomer(scope.row, 2)"
                v-auth="'wmsBaseCustomerPreset:auth'"> 驳回 </el-button>
            </template>
          </el-table-column>
        </el-table>
