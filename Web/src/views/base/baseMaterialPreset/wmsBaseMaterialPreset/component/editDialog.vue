<script lang="ts" name="wmsBaseMaterialPreset" setup>
...
// 页面加载时
onMounted(async () => {
  const data = await wmsBaseMaterialPresetApi.getDropdownData(false).then(res => res.data.result) ?? {};
...
const openDialog = async (row: any, title: string) => {
	state.title = title;
	row = row ?? {  };
	state.ruleForm = row.id ? await wmsBaseMaterialPresetApi.detail(row.id).then(res => res.data.result) : JSON.parse(JSON.stringify(row));
...
const submit = async () => {
	ruleFormRef.value.validate(async (isValid: boolean, fields?: any) => {
		if (isValid) {
			...
			await wmsBaseMaterialPresetApi[state.ruleForm.id ? 'update' : 'add'](values);
			closeDialog();
		} else {
			ElMessage({
...
<template>
	<div class="wmsBaseMaterialPreset-container">
